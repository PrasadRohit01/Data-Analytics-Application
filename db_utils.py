...
def _needs_tls(config) -> bool:
    """Managed cloud databases (TiDB Cloud, RDS, PlanetScale, ...) require
    TLS; a local dev MySQL on localhost/127.0.0.1 normally doesn't have a
    cert configured at all. Use the host as a simple, zero-config signal
    rather than adding another widget/setting to fill in."""
    return config.mysql_host not in ("localhost", "127.0.0.1")


def _ssl_args(config) -> dict:
    return {"ssl": {"ca": certifi.where()}} if _needs_tls(config) else {}


def get_engine(config):
    return create_engine(
        config.mysql_url, pool_pre_ping=True, connect_args=_ssl_args(config)
    )
...
def ensure_database_exists(config):
    ...
    if _needs_tls(config):
        connect_kwargs["ssl_ca"] = certifi.where()
